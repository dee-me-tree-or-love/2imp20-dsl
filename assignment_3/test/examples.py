...
//			Unit 	- a set of predefined units in the type system
//		Singulars:
//			Boolean
...
// Allows to access predefined/external definitions.
// In this example, `Commons`, is a different module defined earlier.
...
Actions : {
	...
	check_water : <%src, condition, job> [
		if %src.water meets condition then do job else {}
	],
	// multiple statements + variables
	check_avg_temp : <%src, condition, job> [
		n = %src.items @> {_, + 1};
		total = %src.items @> {i, + i.temp};
		if total/n meets condition then do job;
	],
	// composition of multiple actions
	check_water_and_temp : <
		%src, 
		condition_water, 
		condition_temp, 
		job // job, as can be seen is any bas
	> [
		if (
			check_water : {condition_water, {}} 
			|& check_avg_temp : {condition_temp, {}}
		)
		then do
		vjob.
		// Additional composition operations for the future:
		// `||` = or (try first, if fails, try next)
		// `|>` = feed (pass output to)
		// `|*` = run in parallel
		// Supporting brackets like
...
Assets : {
	// This defines metadata of any Construct.
